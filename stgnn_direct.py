...
STGNN-direct：直接上游-下游河网图版本。
- 节点：16 个子流域 + 1 个虚拟出口 output。
- 边：WatershedInfo_new.xlsx 中的直接边，例如 1->3, 2->3, ..., 16->output。
...
def build_direct_graph(info_file: str,
                       edge_weight_mode: str,
                       n_subbasins: int = 16):
    direct_edges = parse_edge_table(info_file)
    edge_index, edge_attr, edge_weight, used_df = build_edge_tensors(direct_edges, edge_weight_mode, n_subbasins)
...
def main():
    ...
    # 用户需要改的设置都集中放在这里
    # -------------------------------------------------------------------------
    MODEL_NAME = "stgnn_direct"
    MODEL_LABEL = "STGNN-direct"
    ...
    OUTPUT_DIR = "results_stgnn_direct"
    N_SUBBASINS = 16
    ...
    # direct 图只连接直接上下游。若要让最远上游同日信息传到 output，GNN 层数需不小于最大拓扑跳数。
    # 本研究的16子流域图中，1/2 -> ... -> output 约需 6 跳，所以 direct 版本默认给 6 层。
    GNN_LAYERS = 6
    HIDDEN_DIM = 32
    DROPOUT = 0.40
    ...
    edge_index, edge_attr, edge_weight, edge_df_used = build_direct_graph(info_path, EDGE_WEIGHT_MODE, N_SUBBASINS)
    edge_df_used.to_csv(out_dir / "edge_table_used.csv", index=False, encoding="utf-8-sig")
    print(f"Direct graph: {edge_index.shape[1]} 条边，EDGE_WEIGHT_MODE={EDGE_WEIGHT_MODE}")
